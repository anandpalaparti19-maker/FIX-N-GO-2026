...
class FindingTechScreen extends StatefulWidget {
  const FindingTechScreen({super.key});
...
class _FindingTechScreenState extends State<FindingTechScreen>
  ...
  void initState() {
    ...
    // Auto-find tech after 4 seconds
    Future.delayed(const Duration(seconds: 4), () {
      if (mounted) setState(() => _techFound = true);
    });
  }
                    ...
                    ? _TechFoundCard(
                        onTrack: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (_) => const TrackTechnicianScreen()),
                          );
                        },
                      )
                    : _SearchingCard(),
...
class _TechFoundCard extends StatelessWidget {
  ...
  const _TechFoundCard({required this.onTrack});
